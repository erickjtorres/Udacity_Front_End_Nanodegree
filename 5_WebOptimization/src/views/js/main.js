...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    //Note for changing the pizzasizes we only need to know the length of one so we do not have
    //to itterate through all the pizzas --> we can take dx and newwidth out of the loop for this reason
    //Changing the pizza sizes we will also change the querySelectorAll to getElementsByClassName
    var dx = determineDx(document.getElementsByClassName("randomPizzaContainer")[0], size);
    var newwidth = (document.getElementsByClassName("randomPizzaContainer")[0].offsetWidth + dx) + 'px';
    //then we can set the width of all the pizzas to the new width
...
//we need to uptimize this function because if you look at the timeline you will see that the "bars" are above
//the 30fps line by a lot
function updatePositions() {
  frame++;
  window.performance.mark("mark_start_frame");

  //This is getting all the pizzas that are animating because they all have a class named mover
  //instead of querySelectorAll we can use something more efficient such as getElemntsByClassName
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    /*note we also only have five number to use because when we take the modulus of something
    we get the remainer  -> essentially meaning we will not have 200 phases but less
    --> to solve this we will make the phase variable an array above and append the elements then
    in another for loop we will use this values*/
    phase.push(Math.sin((scrollTop / 1250) + (i % 5)));
  }

  for (var k = 0; k < items.length; k++) {
    items[k].style.left = items[k].basicLeft + 100 * phase[k]+ 'px';
  }
...
//this event listener is listening to when the user scrolls
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //we dont really need to render 200 pizzas because we dont see all of them
  //at most we will see around 30 so we will have i < 30 instead of 200
  for (var i = 0; i < 30; i++) {
